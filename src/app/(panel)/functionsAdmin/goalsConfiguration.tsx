...
import { styles } from "@/styles/functions/goalsConfigurationStyles";
import { GoalFormData, GoalType, Period } from "@/types/goalsTypes/goals";
import {
...
export default function GoalsConfiguration() {
  ...
  // Toggle para notificações de uma meta específica
  const toggleNotifications = (goalKey: string) => {
    setNotificationsEnabled((prev) => ({
      ...prev, // mantém o estado das outras metas
      [goalKey]: !prev[goalKey], // inverte o toggle da meta específica
    }));
  };
  ...
  const handleConfirmDeactivate = async () => {
    ...
    const success = await deleteGoal(
      goalToDeactivate as "daily" | "weekly" | "monthly"
    );
  ...
  const renderIcon = (
    ...
    switch (iconFamily) {
      ...
      case "SimpleLineIcons":
        return (
          <SimpleLineIcons name={iconName as any} size={size} color={color} />
        );
      case "Entypo":
  ...
  useEffect(() => {
    ...
    goalTypes.forEach((goalType) => {
      ...
      initialFormData[goalType.key] = {
        targetValue:
          goal && goal.isActive
            ? formatToBrazilianCurrency(goal.goalValue)
            : "",
        isActive: isActive, // Só ativo se existe meta no banco
      };

      // Inicializar estado de notificações com dados da API
      initialNotifications[goalType.key] = goal ? goal.notification : false;
  ...
  const handleSaveGoal = async (goalType: string) => {
    ...
    const shouldNotify = notificationsEnabled[goalType] || false;

    if (!data.targetValue || numericValue <= 0) {
      showFeedback("Por favor, insira um valor maior que zero para a meta.", "error");
      return;
    }

    const success = goals[`${goalType}Goal` as keyof typeof goals]
      ? await updateGoal(
          goalType as "daily" | "weekly" | "monthly",
          data,
          shouldNotify
        )
      : await saveGoal(
          ...
          {data.isActive && (
            <>
              ...
              <View style={styles.notificationCard}>
                <View style={styles.notificationHeader}>
                  ...
                  <TouchableOpacity
                    style={[
                      styles.toggle,
                      notificationsEnabled[goalType.key] && styles.toggleActive,
                    ]}
                    onPress={() => toggleNotifications(goalType.key)}
                  >
                    <View
                      style={[
                        styles.toggleThumb,
                        notificationsEnabled[goalType.key] &&
              ...
              <View style={styles.buttonsContainer}>
                <TouchableOpacity
                  style={[
                    hasActiveGoalFromDB ? styles.updateButton : styles.saveButton,
                    saving && (hasActiveGoalFromDB ? styles.updateButtonDisabled : styles.saveButtonDisabled),
                  ]}
                ...
                >
                  <Text
                    style={[
                      hasActiveGoalFromDB ? styles.updateButtonText : styles.saveButtonText,
                      saving && (hasActiveGoalFromDB ? styles.updateButtonTextDisabled : styles.saveButtonTextDisabled),
                    ]}
                  >
                    {hasActiveGoalFromDB ? "Atualizar Meta" : "Salvar Meta"}
                  </Text>
                </TouchableOpacity>
        ...
        >
          <View style={styles.infoCard}>
            ...
            <Text style={styles.infoText}>
              ...
              Metas muito altas podem desmotivar, enquanto metas muito baixas
              não incentivam o crescimento.
            </Text>
          </View>
