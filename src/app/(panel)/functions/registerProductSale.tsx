...
export default function RegisterProductSale() {
  ...
  const { searchByBarcodeInDatabase, loading: barcodeLoading } =
    useBarcodeSearch();
  const {
    getFromCache,
    updateSoldQuantity,
      ...
      setTimeout(() => {
        ...
        if (cachedProduct) {
          ...
          setSelectedProductForDetail({
            ...
            unitPrice:
              parseFloat(result.data?.unitPrice?.toString() || "0") || 0,
  ...
  const handleProductDetailRegister = (product: any, quantity: number) => {
    ...
    if (existingProductIndex !== -1) {
      ...
      const availableStock = cachedProduct
        ? cachedProduct.availableStock
        : product.quantity;

      console.log(
        "🛒 Estoque disponível:",
        product.productName,
        `cache: ${cachedProduct?.availableStock || "N/A"}`,
        `fallback: ${product.quantity}`,
        `usando: ${availableStock}`
      );
  ...
  const updateProductQuantity = (productId: string, newQuantity: number) => {
    ...
    if (newQuantity > totalStockAvailable) {
      showFeedback(
        `Quantidade não pode ser maior que o estoque disponível (${totalStockAvailable})`,
        "warning"
      );
      return;
    }
  ...
  const removeProduct = (productId: string) => {
    ...
    if (productToRemove) {
      showFeedback(
        `${productToRemove.quantity} unidade(s) de ${productToRemove.name} removida(s)`,
        "info"
      );
    }
  };
  ...
  const renderProductItem = ({ item }: { item: ProductItem }) => {
    ...
    return (
      ...
      >
        ...
        {!isExpanded && (
          <Text style={styles.productCardQuantity}>Qtd: {item.quantity}</Text>
        )}
